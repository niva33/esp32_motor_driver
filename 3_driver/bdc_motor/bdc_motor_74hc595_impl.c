...
static esp_err_t shift_out(uint32_t _data_pin, uint32_t _clock_pin, uint8_t _data)
{
    gpio_set_level(_latch_pin, 0);
    // for()
    gpio_set_level(_latch_pin, 1);
...
static esp_err_t bdc_motor_mcpwm_disable(bdc_motor_t *motor)
{
    ...
    gpio_set_level(mcpwm_motor->shift_reg->latch_gpio_num, 0);
    shift_out(mcpwm_motor->shift_reg->data_gpio_num, mcpwm_motor->shift_reg->clk_gpio_num, (uint8_t)ST);
    gpio_set_level(mcpwm_motor->shift_reg->latch_gpio_num, 1);
...
static esp_err_t bdc_motor_mcpwm_forward(bdc_motor_t *motor)
{
    ...
    gpio_set_level(mcpwm_motor->shift_reg->latch_gpio_num, 0);
    shift_out(mcpwm_motor->shift_reg->data_gpio_num, mcpwm_motor->shift_reg->clk_gpio_num, (uint8_t)M1_FW);
    gpio_set_level(mcpwm_motor->shift_reg->latch_gpio_num, 1);
    return ESP_OK;
}
...
static esp_err_t bdc_motor_mcpwm_reverse(bdc_motor_t *motor)
{
    bdc_motor_mcpwm_obj *mcpwm_motor = __containerof(motor, bdc_motor_mcpwm_obj, base);
    gpio_set_level(mcpwm_motor->shift_reg->latch_gpio_num, 0);
    shift_out(mcpwm_motor->shift_reg->data_gpio_num, mcpwm_motor->shift_reg->clk_gpio_num, (uint8_t)M1_BW);
    gpio_set_level(mcpwm_motor->shift_reg->latch_gpio_num, 1);
    return ESP_OK;
}
...
static esp_err_t bdc_motor_mcpwm_coast(bdc_motor_t *motor)
{
    ...
    gpio_set_level(mcpwm_motor->shift_reg->latch_gpio_num, 0);
    shift_out(mcpwm_motor->shift_reg->data_gpio_num, mcpwm_motor->shift_reg->clk_gpio_num, (uint8_t)M1_ST);
    gpio_set_level(mcpwm_motor->shift_reg->latch_gpio_num, 1);
    return ESP_OK;
}
...
esp_err_t bdc_motor_new_mcpwm_device(const bdc_motor_config_t *motor_config, const bdc_motor_mcpwm_config_t *mcpwm_config, bdc_motor_handle_t *ret_motor)
{
    ...
    io_conf.pin_bit_mask = motor_config->shift_reg_config_t->latch_gpio_num;
    ...
    io_conf.intr_type = GPIO_INTR_DISABLE;
    io_conf.mode = GPIO_MODE_OUTPUT;
    io_conf.pin_bit_mask = motor_config->shift_reg_config_t->clock_gpio_num;
    io_conf.pull_down_en = 0;
    io_conf.pull_up_en = 0;
    gpio_config(&io_conf);
    ...
    io_conf.intr_type = GPIO_INTR_DISABLE;
    io_conf.mode = GPIO_MODE_OUTPUT;
    io_conf.pin_bit_mask = motor_config->shift_reg_config_t->data;
    io_conf.pull_down_en = 0;
    io_conf.pull_up_en = 0;
    gpio_config(&io_conf);
    ...
    mcpwm_generator_config_t generator_config = {
        .gen_gpio_num = motor_config->pwm_gpio_num,
    };
